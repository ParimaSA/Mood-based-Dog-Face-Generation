...
Classical computer vision methods rely on hand-crafted features like shape, color, and landmarks. They are interpretable and simple but struggle with changes in pose, lighting, and subtle expressions. Similarly, rule-based and feature-engineered methods depend on human-defined logic and expertise, making them limited in flexibility and poor at generalizing to real-world scenarios.

Deep learning offers a better approach by learning features directly from data. In particular, conditional GANs (cGANs) can generate realistic dog face images based on mood labels. The generator creates images while the discriminator evaluates them, enabling the model to produce diverse, labeled data. This makes cGANs especially useful for data augmentation despite challenges like training instability and data requirements.
...
Fréchet Inception Distance (FID) was used to evaluate image quality. FID slightly improved from 193.20 at epoch 100 to 186.88 at epoch 200, with minor fluctuations in between. Although this shows modest improvement, the scores remain relatively high, indicating that generated images are still noticeably different from real ones. Visually, the model produces dog‑like shapes, colors, and textures, but outputs are often blurry and emotional differences between classes are not clearly visible.
